...
class _WristState:
    ...
    recent_speeds: deque[float] = field(default_factory=deque)
...
class SpeedEngine(MetricsEngine):
    ...
    def _step(
        ...
        speed = self._calibration.scale_speed(distance_px / dt)
        state.last_position_px, state.last_timestamp_s = position_px, timestamp_s

        state.recent_speeds.append(speed)
        while len(state.recent_speeds) > self._config.smoothing_window:
            state.recent_speeds.popleft()
        smoothed = sum(state.recent_speeds) / len(state.recent_speeds)

        if not state.in_stroke:
            if smoothed >= self._start_speed:
                state.in_stroke = True
                state.stroke_start_s = timestamp_s
                state.peak_speed = smoothed
            return

        state.peak_speed = max(state.peak_speed, smoothed)
        stroke_over = smoothed <= self._config.end_speed_ratio * state.peak_speed
    ...
    def _close_stroke(
        ...
        qualifies = (
            state.peak_speed >= self._peak_min_speed
